# VCC Funding Control Index

Prepared: August 1, 2026

Company: VILIGANS COMMAND CORPORATION

Operating path: DOCS 8404 Logistics & NEMT LLC

Corporate contact identity: aldavis@viliganscommandcorp.com

## Executive Summary

VCC Funding is the active control lane for moving the Wyoming rural special-needs transportation pilot from planning to funded launch. The practical funding sequence is partner revenue first, sponsorship second, lender/vehicle readiness third, and grant scale-up through eligible direct or partner-led paths.

The fastest fundable offer is a 90-day Wyoming pilot with a monthly readiness fee, per-trip charge, limited service area, weekly reporting, accessible vehicle readiness, insurance and authority gates, and partner referral workflow.

## Immediate Actions

| Priority | Action | Owner | Due | Impact |
|---:|---|---|---|---|
| 1 | Select one pilot county/service area and one anchor partner category. | CEO | Immediate | High |
| 2 | Convert the 90-day budget into a quote-backed funding ask. | CEO/Finance | 7 days | High |
| 3 | Request commercial auto and general liability quotes for the selected vehicle/use profile. | CEO/Broker | 7 days | High |
| 4 | Send approved outreach to 20 partner targets and 20 sponsor targets after approval. | CEO/Marketing | 10 days | High |
| 5 | Confirm direct vs partner-led path for ICAM, WYDOT/FTA, and other public funding. | CEO/Grant lead | 10 days | High |
| 6 | Build the first lender/sponsor packet from approved summary, capability statement, budget, evidence register, and partner MOU. | CEO/Ops | 14 days | Medium |

## Funding Opportunities

| Route | Timing | Best VCC Position | Next Action |
|---|---|---|---|
| Anchor partner contract | Immediate | Operator/service provider | Secure LOI, MOU, readiness fee, or paid pilot agreement. |
| Local sponsorship | Immediate | Sponsored operator | Package ride blocks and vehicle readiness sponsorships. |
| RV / mobile outreach sponsorship | Immediate after approval | Sponsored outreach asset, not primary passenger vehicle unless separately approved | Use the RV action plan to price mobile outreach days, vehicle-readiness sponsorship, and community intake events. |
| FTA ICAM | Due September 9, 2026 | Applicant, partner, operator, or subrecipient depending on eligibility | Confirm eligibility path and secure partner letters. |
| FHWA ATTAIN | Due August 11, 2026 | Technology/mobility partner if a qualified public-sector lead is ready | Proceed only if a lead partner is already available. |
| Wyoming Community Foundation | Fall 2026 | Partner/operator/project beneficiary if direct eligibility is limited | Confirm current deadline and applicant eligibility. |
| WYDOT/FTA 5310/5311/5339 | Next cycle or partner path now | Operator/subrecipient/contractor where permitted | Contact WYDOT Transit about applicant path, match, and vehicle capital. |
| VA HRTG | Next cycle | Operator/subrecipient/contractor under qualified lead where permitted | Build VSO/SVSA MOU packet and rural veteran ride need statement. |
| Bank/lender facility | After LOI, quote, and vehicle plan | Borrower/operator | Prepare underwriting packet and cash-flow support. |

## Compliance Requirements

| Area | Required Before External Use |
|---|---|
| Corporate authority | Signed bylaws, officer authority, board consent, stock ledger, and cap table as applicable. |
| Federal readiness | SAM/UEI, Grants.gov access, and accurate certification language before grant submissions. |
| Transportation authority | WYDOT/FMCSA path documented before compensated passenger transportation. |
| Insurance | Commercial auto, GL, professional/operational coverages, and endorsements before operation. |
| Vehicle readiness | Title/lease, inspection, accessibility equipment records, maintenance plan, and registration. |
| RV purchase control | Use classification, title, lien, inspection, insurance bindability, ownership, financing, storage, and no-personal-use controls before any purchase. |
| Data controls | HIPAA/BAA and sharing-control review before storing rider medical, Medicaid, disability, or trip-purpose health information in Workspace. |
| External commitments | Approval required before contracts, MOUs, sponsorships, lender applications, grants, insurance bind orders, or public launch statements. |

## Business Risks

| Risk | Level | Control |
|---|---|---|
| Pre-award grant cost assumption | High | Do not rely on reimbursement unless grantor authorizes costs in writing. |
| For-profit direct grant eligibility limits | High | Use eligible public, nonprofit, transit, VSO, or community partner paths where needed. |
| Underinsured passenger service | Critical | No compensated passenger service until insurance and authority gates are complete. |
| Unsupported public claims | High | Use draft/readiness language until certifications, authority, insurance, and launch gates are verified. |
| Working capital shortage | High | Secure anchor revenue, sponsors, and reserve before vehicle or staffing commitments. |
| Inconsistent ownership narrative | Medium | Use VCC and DOCS 8404 corporation/LLC authority language consistently. |

## Revenue Opportunities

| Revenue Path | Offer |
|---|---|
| Anchor partner readiness fee | Fixed monthly fee for reserved pilot capacity and reporting. |
| Per-trip contract revenue | Zone or mileage pricing after authority and insurance gates. |
| Sponsor-funded ride blocks | Community, senior, disability, healthcare access, and founding mobility sponsor packages. |
| Employer access pilot | Scheduled rides for workforce access where legally and operationally permitted. |
| County or clinic support contract | Limited service-area support with weekly metrics. |
| Lender-supported vehicle acquisition | Quote-backed underwriting once partner revenue or sponsor commitments exist. |
| Mobile outreach RV sponsorship | Sponsored intake events, community mobility needs assessment, partner fairs, and brand-visible ride-access campaigns after approval. |

## Partnership Opportunities

| Partner Type | Target Ask |
|---|---|
| Hospital or clinic | LOI, referral workflow, readiness fee, support letter. |
| Dialysis or rehab provider | Recurring ride demand validation and paid pilot discussion. |
| County public health | Needs validation, coordination support, grant partnership. |
| Senior center | Referral partnership and sponsor-supported ride block. |
| Disability provider | Need statement, accessibility feedback, rider workflow. |
| VSO/SVSA | Veteran transportation support letter and grant lead discussion. |
| Local employer | Workforce access pilot or sponsor package. |

## Next Steps

1. Review and approve the funding action register.
2. Update the 90-day budget with real insurance, vehicle, driver, and compliance quotes.
3. Approve outreach scripts and partner MOU language before external contact.
4. Build one sponsor sheet from the funding stack and impact metrics.
5. Confirm ICAM and WYDOT/FTA applicant path before grant workspace creation.
6. Move approved evidence into the appropriate VCC-owned Shared Drive after Workspace controls are verified.
